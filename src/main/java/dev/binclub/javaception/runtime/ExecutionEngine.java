...
import dev.binclub.javaception.runtime.nativehooks.DesiredAssertionStatus0;
import dev.binclub.javaception.runtime.nativehooks.ForName0;
...
public class ExecutionEngine {
	...
	public static Map<String, IHookedMethod> nativeHooks = new HashMap<>();
	public static HashSet<String> allowedNativeMethods = new HashSet<>();
	
	static {
		nativeHooks.put("forName0(Ljava/lang/String;ZLjava/lang/ClassLoader;Ljava/lang/Class;)Ljava/lang/Class;", new ForName0());
		nativeHooks.put("desiredAssertionStatus0(Ljava/lang/Class;)Z", new DesiredAssertionStatus0());
		allowedNativeMethods.add("isBigEndian()Z");
	}
	...
	public static Object invokeMethodObj(Klass owner, InstanceOop instance, MethodInfo method, Object... args) {
		System.out.println(owner.name + method.name + method.signature);
		if ((method.access & ACC_NATIVE) != 0) {
			if (method.descriptor[method.descriptor.length - 1] == PrimitiveType.VOID) {
				System.out.println("Skipping native method");
				return null;
			}
			String lookupName = method.name + method.signature;
			IHookedMethod hook = nativeHooks.get(lookupName);
			if (hook != null) {
				return hook.invokeMethodObj(owner, instance, method, args);
			}
			if (Modifier.isStatic(method.access) && allowedNativeMethods.contains(lookupName)) {
				try {
					Class<?> klazz = Class.forName(owner.name.replace("/","."));
					Class<?>[] params = new Class<?>[method.descriptor.length - 1];
					for (int i = 0; i < params.length; i++) {
						Type type = method.descriptor[i];
						if (type instanceof PrimitiveType) {
							params[i] = ((PrimitiveType) type).jvmClass;
						} else {
							params[i] = Class.forName(type.toString());
						}
					}
					Method nativeMethod = klazz.getDeclaredMethod(method.name, params);
					if(!nativeMethod.canAccess(null)){
						nativeMethod.setAccessible(true);
					}
					if(method.descriptor[method.descriptor.length - 1] != PrimitiveType.VOID){
						Object ret = nativeMethod.invoke(null, args);
		...
		System.out.println("Exiting " + (owner.name + method.name + method.signature));
	...
	public static Object makeArray(byte type, int size){
		switch (type){
			case 4 ->{
